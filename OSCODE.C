...
printf("Enter 2 to check the sequence when additional resources are requested byP[0]\n");
p
rintf("Enter 3 to check the sequence when additional resources are requested byP[1]\n");
scanf("\n%d",&n);
switch(n)
{
case 1:
printf("\n\n");
printf("The safe sequence for the above given processes if additional instances are notrequested ::\n");
printf("\n");
while(temp1!=process)
{
for(i=0;i<process;i++)
...
printf("So Reenas operating system is unsafe if there is a additionalrequest by P[0]\n");
break;
}
}
break;
